...
- [x] **Background refresh**: If cache exists but is stale, the endpoint returns the stale cache immediately. Background refresh is handled by the `dashboard:refresh-stats` GitHub Actions cron (hourly). Note: `defer()` was originally used but removed (commit 346b29d) because it blocks HTTP responses on Dreamhost shared hosting.
