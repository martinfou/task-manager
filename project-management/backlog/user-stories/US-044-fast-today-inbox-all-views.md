...
- [x] **Cache-first with stale indicator**: The endpoint returns cached data immediately. If the cache is stale (past configurable TTL, default 5 minutes), a background refresh is triggered. The response includes a `cachedAt` timestamp so the frontend can optionally show "as of …".
