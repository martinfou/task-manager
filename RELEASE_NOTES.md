...
- **Undo toast** — After complete, deferred single-task delete, or moving exactly one selected task, a toast offers **Undo**; delay from config / Profile; one toast at a time; `role="status"`; EN/FR. See [apps/google-tasks/docs/UNDO_TOAST.md](apps/google-tasks/docs/UNDO_TOAST.md). [US-029](project-management/backlog/user-stories/US-029-undo-toast-destructive-actions.md)
